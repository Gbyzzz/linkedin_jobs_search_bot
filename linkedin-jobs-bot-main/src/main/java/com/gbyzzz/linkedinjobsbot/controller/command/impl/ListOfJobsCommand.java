...
//                Long searchParamsId = command[2].equals(MessageText.ALL) ? null : Long.parseLong(command[2]);
                sendMessage = messageService.getNewJobByUserId(id, update.getCallbackQuery().getData());
//                        Long.parseLong(command[4]), searchParamsId, Integer.parseInt(command[3]), command[1]);
            }
        }
//        Long targetId = null;
//
...
//                SavedJob savedJob = savedJobService.getJobByIdAndUserId(targetId, id);
...
//            switch (state) {
//                case MessageText.NEW -> {
//                    sendMessage = messageService.getNewJobByUserId(id, index);
////                            new SendMessage(id.toString(),
////                            MessageText.makeNewJobsReply(index, jobs));
//                    sendMessage.setReplyMarkup(paginationKeyboard.getReplyButtons(
//                            index, jobs.size(), state, searchParamsId));
//                }
//                case MessageText.APPLIED -> {
//                    sendMessage = messageService.getAppliedJobByUserId(id, index);
////                    new SendMessage(id.toString(),
...
//                }
//                case MessageText.SEARCHES -> {
//                    sendMessage = messageService.getSearchByUserId(id, index);
////                            new SendMessage(id.toString(),
